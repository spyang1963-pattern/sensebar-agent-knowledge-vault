...
排序最前面的 5 檔是你最有把握的「核心推薦」，在股名後加【核心】標記（例：「2330 台積電【核心】」），其餘為觀察名單。

⚠️ 大盤方向一致性鐵律（最重要，不可違反）：
- 先依「## 明日大盤情境」判斷明日大盤偏強/偏弱，個股方向必須與大盤情境自洽。
- 大盤偏弱時，榜單「偏空/中性」檔數應多於「偏多」檔；每檔「偏多」股必須有明確「逆勢抗跌」獨立依據（籌碼極度集中、獨立題材催化），否則列偏空或中性。
- **「資金位移 verdict=進貨/惜售」是「今日」資金狀態，不是明日漲跌的充分條件**。大盤重挫日，資金流入股照樣被拖累下跌，嚴禁機械式把「進貨」直接翻成「偏多」。
- 產出前自我核對：榜單方向分布是否與「明日大盤情境」一致？偏多股是否都有抗跌依據？

⚠️ 資金板塊權重鐵律（本儀表板核心資訊，不可違反）：
- input「## 1 XQ 盤中快照摘要」的「資金位移（成交值增減前8）」列出 verdict=進貨/惜售/退潮/疑似出貨。其中 **verdict=進貨或惜售（資金流入、量價配合）的個股，應占預測榜至少一半（≥6 檔）**，因為它們是盤面主力資金的落點。
