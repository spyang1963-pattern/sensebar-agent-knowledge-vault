...
## 明日個股預測榜
挑選 20 檔：**偏多 10 檔 ＋ 偏空 10 檔**，中性股不列入。偏多組與偏空組各自涵蓋大型/中型/小型三類（市值參考 input「## 0 現價速查表」的市值欄）：
- 大型：市值 ≥ 800 億
- 中型：市值 100~800 億
- 小型：市值 < 100 億
...
⚠️ 強勢/弱勢選股鐵律（不可違反）：
- 預測榜目標是「**明日日內趨勢明確**」的個股，不是「成交值最大的權值股」。**嚴禁直接拿成交值前 20 名填榜單**——那些多半是日K 盤整、均線糾結的權值股。
- 偏多組：從「## 0c 均線排列速查表」的**強勢股（全多排列＝MA8>MA21>MA55 且三者向上）**中挑；15分K 站上 8MA、5分K 共振起漲者優先。
- 偏空組：從**弱勢股（全空排列＝MA8<MA21<MA55 且三者向下）**中挑；15分K 跌破 8MA、5分K 共振下跌者優先。
- **均線糾結、盤整、無明確趨勢的個股一律不選**（即使成交值很大、是權值股）。選不出趨勢明確的就寧可少列，不可硬湊 20 檔。
