...
## 明日個股預測榜
挑 12~15 檔，按「值得留意程度」排序。**規模必須涵蓋大型/中型/小型三類，且至少 4~5 檔屬中小型股**（市值 < 800 億；市值參考 input「## 0 現價速查表」的市值欄）。每檔**嚴格**用以下格式（標題行與細節行都要照寫，不可省略任何欄位）：

N. 代碼 名稱｜方向：偏多/偏空/中性｜強度：強/中/弱｜規模：大型/中型/小型
- 出榜依據：條列 2~4 條具體實數（例如「資金位移 verdict=進貨、成交值 +X 億」「外資買超 X 張、投信買超 Y 張」「融資 -X 張、券資比 Y%」「千張大戶 +Zpp」），每條都要有數字，**禁止**寫「法人看好」「資金流入」這種無數字空話
- 可靠度：高/中/低＋一句話理由（多個指標同步同向者「高」，僅單一指標或訊號互斥者「中/低」）
