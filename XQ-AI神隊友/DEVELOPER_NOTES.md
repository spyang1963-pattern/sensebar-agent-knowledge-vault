...
### 🔬 歸納引擎（2026-09-16，`357fed3`，回應「收集數據卻不歸納問題」的質疑）
- **使用者質疑（設計檢討，重要）**：稽核收集一堆數字（命中率/可靠度/區間/概率），但「歸納錯在哪 → 修正」只靠人偶發做一次（大盤一致性），之後變成「收集數據等人看」，回饋 Gemini 的又是「你命中率 38%、請檢討」這種模糊空話——沒有效率。
- **升級 `predict_audit.py`**：
  - `_score_stocks` 加 `by_dir`（偏多/偏空各自命中）＋`prob_sum`（概率總和）。
  - 新增 `_merge_stats()` 合併最近 N 天。
  - **`recent_summary()` 重寫為歸納引擎**：自動拆解 5 個維度，每個都配「具體修正指令」——
    1. 方向偏差：偏多命中 vs 偏空命中 → 過度看多/看空 → 「大盤弱時偏多減半、偏空加倍」。
    2. 可靠度校準：高 vs 中 → 失真就「只有多指標同步才標高」。
    3. 規模偏差：大/中/小命中 → 某規模 <40% 就「列中性或刪除」。
    4. 區間命中：<40% → 「改用技術位階 ±3%~5%」。
    5. 概率膨脹：標的平均概率 vs 實際命中 差 >15pp → 「概率下修 Xpp」。
- **設計原則（寫給未來 agent）**：稽核的價值不在「累積數據」，在「自動歸納錯誤模式 → 生成可執行指令 → 回饋 → 驗證命中率變化」。回饋文字必須是「具體錯在哪＋怎麼改」，禁止「請檢討」這種空話。
