...
## 四、重大事件深度解讀
針對報告中的重大事件（severity 較高者），逐一拆解。每個事件用 `###` 標題（含事件名稱）；其下用 `####` 依序列出細項：
- 事件核心
- 對股/債/匯/商品的傳導路徑
- 關聯標的（台股代號/美股代號）
- 未來一週可能的走勢
...
## 五、跨日相關性分析
如有近兩日報告，分析近三日市場演變的因果閉環與主軸（例如地緣、債市、資金輪動、台股背離），點出趨勢如何延續或轉折。每個階段／主軸用 `###` 標題。

## 六、分類展望
分別就股市、債市、匯市、商品(油金)、地緣政治，給出短(1週)/中(1月)/長(1季)展望。**每個市場用 `###` 標題**；短/中/長展望用 `####` 標示。資訊不足的項目要誠實說明，不要編造。

## 七、風險與機會
列出本報告中最值得關注的風險（警示）與機會（看好），各列 3-5 項。**每一項都必須是下列固定三層結構，缺一不可，不得省略「標的」「理由」子層**：
- 大分類標題：`- **風險（警示）**`、`- **機會（看好）**`
- 項目：`- **風險／機會名稱**`（頂格粗體，名稱精簡）
- 其下固定兩行子項（兩空格縮排，粗體「標的」「理由」四字必須精確）：
  `  - **標的**：具體標的名稱或代號（沒有則誠實寫「無具體標的」）`
  `  - **理由**：具體數據與邏輯`
「標的」「理由」是系統顯示用的關鍵字，必須原樣寫出這兩個粗體字，不可改成其他詞。

## 八、操作建議摘要
以投資者視角，給出 3-5 條具體、可執行的觀察重點（不是投資建議），並註明需要盯盤的關鍵指標（如油價、美債殖利率、台指期夜盤）。每條觀察用 `###` 開頭。
...
語言風格要求（重要）：
- 讀者是一般普通民眾，不是專業分析師：避免艱澀專業用語；必須使用專業詞時，第一次出現請在後面以括號白話解釋（例：「殖利率（把錢借給政府每年能拿到的利息回報率）」）。
- 表達必須數據化、具體：禁止模糊抽象的說法（如「承壓」「偏高」「可能波動」），一律換成具體數字或對照（例：「從 100 跌到 95（約 -5%）」「高於近三個月平均約 12%」）；每個論點至少附一個報告中的實際數字。
